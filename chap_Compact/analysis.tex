...
In the third adversarial scenario, malicious RPs collude with malicious users, attempting to link the logins from an honest user to different colluding RPs. From each login request, an RP learns an ephemeral identifier $PID_{U}$ and a permanent identifier $Acct = [ID_U]ID_{RP}$ of the user, but it cannot derive $ID_U$ from $Acct$ due to the ECDLP problem. 
Therefore, the colluding adversaries attempt to share information about the logins to malicious RPs or initiated by malicious users to link multiple logins (and the corresponding $PID_{U}$s) initiated by the same honest user to multiple malicious RPs. 
...
With the trapdoor $t$, $PID_{RP}$ and $PID_U$ can be easily transformed into $ID_{RP}$ and $Acct$, respectively, and vice versa. Therefore, we can simply denote the information that an RP learns from a login by a tuple $L$, where $L =(ID_{RP}, t, Acct)=(ID_{RP}, t, [ID_{U}]ID_{RP})=([r]G, t, [ur]G)$. 
Hence, an $RP_j$ with $w$ logins knows $\{L_{1,j},...,L_{w,j}\}$. 

When $c$ malicious RPs in a system of $m$ RPs collude ($c \le m$), they create a shared view of all their logins, denoted as $\mathbb{L}$. When $v$ malicious users collude with these RPs, their logins can be further linked together as:
...
\noindent where $L_{i, j}=([r_j]G, t_{i,j}, [u_ir_j]G)$ for $1 \le i \le v$ and $1 \le j \le c$, and $\mathfrak{L} \subseteq \mathbb{L}$. Any login in $\mathbb{L}$ but not in $\mathfrak{L}$ is initiated by an honest user to a malicious RP. 
...
\vspace{3mm}
\noindent \textsc{Theorem 7. (Privacy against Colluding RPs)} {In \usso, given $\mathbb{L}$ and $\mathfrak{L}$, $c$ malicious RPs and $v$ malicious users cannot link any login from an honest user to a malicious RP to any subset of logins from any honest user to any malicious RP.}
...
\vspace{0.75mm}
\noindent\textsc{Proof.} From logins in $\mathbb{L}$ but not in $\mathfrak{L}$, we randomly choose one login $L_a$, which is from an (unknown) honest user with $ID_{U_a}=u_a$ to a malicious $RP_a$, $a \in [1,c]$. Then, we randomly choose another malicious $RP_b$, $b \in [1,c]$ and $b \neq a$. Its logins $\mathbb{L}_b = \{L_{k,b}\}$, $k>v$, are from unknown honest users. Consider any subset of $\mathbb{L}_b$ with $w$ logins, we denote the corresponding unknown honest users who initiate these logins as $\mathbf{u}_w=\{{u_1}, {u_2}, \cdots, {u_w}\}$. 
Next, we prove that the colluding adversaries cannot decide if $u_a$ is in $\mathbf{u}_w$ or not (i.e., $u_a$ is randomly selected from the universal user set). This indicates that the colluding adversaries cannot link $L_a$ to another login or any subset of $\mathbb{L}_b$. 

To prove this, we first define an RP-based linkage game $\mathcal{G}_r$ between an adversary and a challenger, which describes the RP-based login linkage privacy threat: the adversary receives $\mathfrak{L}$, $L_a$, and $\mathbb{L}_b$ from the challenger and outputs $s$, where $s = 1$ if it decides $u_a$ is in $\mathbf{u}_w$ %$\{{U''_1}, {U''_2}, \cdots, {U''_w}\}$ 
and $s=0$ if it believes $u_a$ is randomly chosen from the universal user set.
...
&{\rm Pr}_1={\rm Pr}(\mathcal{G}_r(\mathfrak{L}, L_a, \mathbb{L}_b)=1 \;| \; u_a \in \mathbf{u}_w)  \\
%&{\rm Pr}_2={\rm Pr}\{\mathcal{G}_r(\mathfrak{L}, L', \mathbb{L}'' | u' \in \mathbb{Z}_n)=1\}\\
&{\rm Pr}_2={\rm Pr}(\mathcal{G}_r(\mathfrak{L}, L_a, \mathbb{L}_b)=1 \; | \; u_a \in \mathbb{Z}_n)\\
&{\mathbf{Adv}}_{A}=|{\rm Pr}_1-{\rm Pr}_2|
\end{align*}

As depicted in Figure \ref{fig:dalgorithm}, we design a PPT algorithm $\mathcal{D}^*_r$ based on $\mathcal{G}_r$ to solve the elliptic curve decisional Diffie-Hellman (ECDDH) problem: given $(G, [x]G$, $[y]G$, $[z]G)$, decide whether $z$ equals to $xy$ or is randomly chosen in $\mathbb{Z}_n$, where $G$ is a point on an elliptic curve $\mathbb{E}$ of order $n$, and $x$ and $y$ are integers randomly and independently chosen in $\mathbb{Z}_n$.
...
%\end{align*}
%
The algorithm $\mathcal{D}^*_r$ works as follows: (1) upon receiving an input $(G, Q_1=[x]G, Q_2=[y]G, Q_3=[z]G)$%of $\mathcal{D}^*_r$
, the challenger randomly chooses $a \in [1, c]$ and $t_a \in \mathbb{Z}_n$ to assemble $L_a = ([r_{a}]G, t_a, [r_{a}]Q_1) = ([r_{a}]G, t_a, [xr_{a}]G)$. (2) It then chooses random numbers in $\mathbb{Z}_n$ to construct $\{u_i\}$, $\{r_j\}$, and $\{t_{i, j}\}$, for $1 \le i \le v$ and $1 \le j \le c$, with which it assembles $L_{i, j}=([r_j]G, t_{i,j}, [u_ir_j]G)$. In this process, it ensures $r_j \neq y$, so that $[r_{j}]G \neq Q_2$. 
% Here, $L'$ represents the knowledge of the login visiting $RP_{j'}$ by a user with $ID_U = x$.
Next, the challenger randomly chooses $b \in [1, c]$ and $b \neq a$ and replaces $ID_{RP_b}$ with $Q_2 = [y]G$. Hence, for any $L_{i,b}$, $1 \le i \le v$, the challenger replaces $L_{i, b}=([r_b]G, t_{i,b}, [u_ir_b]G)$ with $(Q_2, t_{i,b}, [u_i]Q_2) = ([y]G, t_{i,b}, [u_iy]G)$ and constructs $\mathfrak{L}$. 
(3) The challenger chooses random numbers in $\mathbb{Z}_n$ to construct $\{u_k\}$ and $\{t_k\}$, for $1 \leq k \leq w$, from which it assembles $L_{v+k,b} = (Q_2, t_k, [u_k]Q_2) = ([y]G, t_k, [u_ky]G)$. In this process, the challenger ensures that $u_k \neq x$ (i.e., $[u_k]G \neq Q_1$) and $u_k \neq u_i$ for any $1 \le i \le v$. Then, it randomly chooses $d \in [1, w]$ and replaces $L_{v+d,b}$ with $(Q_2, t_d, Q_3) = ([y]G, t_d, [z]G)$. Now, the challenger constructs $\mathbb{L}_b = \{L_{v+k,b}\}$, representing logins from $w$ users, i.e., $\mathbf{u}_w=\{u_1, u_2, \cdots, u_{d-1}, z/y, u_{d+1}, \cdots, u_w\}$. (4) When the adversary receives $\mathfrak{L}$, $L_a$, and $\mathbb{L}_b$, it returns $s$ as the output of $\mathcal{D}^*_r$.

According to the above construction, % of $\mathfrak{L}$, $L'$ and $\mathbb{L}''$,
$x$ is embedded as $ID_{U_a}$ in the login $L_a$ to the RP with $ID_{RP_{a}} = [r_{a}]G$, while $z/y$ is embedded as $ID_{U_d}$ in $\mathbb{L}_b$ to the RP with $ID_{RP_{b}}=[y]G$ , together with $\{u_1, \cdots, u_{d-1}, u_{d+1}, \cdots, u_w\}$. Meanwhile, $[r_{a}]G$ and $[y]G$ represent two malicious RPs in $\mathfrak{L}$. Because $x$ is not in $\{u_1, \cdots, u_{d-1}, u_{d+1}, \cdots, u_w\}$ (i.e., $u_k \neq x$), the adversary outputs $s=1$ and succeeds in the game \emph{if and only if} $x = z/y$. Therefore, using $\mathcal{D}^*_r$ to solve the ECDDH problem, the adversary has an $\mathbf{Adv}^*_{A}=|{\rm Pr}^*_1 - {\rm Pr}^*_2|$, where 
...
\begin{align*}
&{\rm Pr}^*_1 =  {\rm Pr}(\mathcal{D}^*_r(G, [x]G, [y]G, [xy]G)=1) \\
=&{\rm Pr}(\mathcal{G}_r(\mathfrak{L}, L_a, \mathbb{L}_b)=1 \; | \; u_a \in \mathbf{u}_w) = {\rm Pr}_1 \\
&{\rm Pr}^*_2= {\rm Pr}(\mathcal{D}^*_r(G, [x]G, [y]G, [z]G)=1) \\
=&{\rm Pr}(\mathcal{G}_r(\mathfrak{L}, L_a, \mathbb{L}_b)=1 \; | \; u_a \in \mathbb{Z}_n) = {\rm Pr}_2 \\
&\mathbf{Adv}^*_{A}=|{\rm Pr}^*_1-{\rm Pr}^*_2|=|{\rm Pr}_1-{\rm Pr}_2|={\mathbf{Adv}}_{A}
\end{align*}
...
If the adversary has non-negligible advantage in $\mathcal{G}_r$, then $\mathbf{Adv}^*_{A}={\mathbf{Adv}}_{A}$ is non-negligible regardless of the security parameter $\lambda$. This violates the ECDDH assumption. Therefore, the adversary has no advantage in $\mathcal{G}_r$ and cannot decide whether $L_a$ is initiated by a user in $\mathbf{u}_w$ or in the universal user set. Because $RP_b$ can be any malicious RP, this proof can be easily extended from $RP_b$ to all $c$ malicious RPs.
\hfill $\square$
